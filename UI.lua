...
local RQ = http_request or http.request or request
...
March.Name = "BSOD_" .. tostring(Random(-100000, 999999))
...
coroutine.wrap(function()
    RunService.RenderStepped:Connect(function(v)
        ...
        library.ping = math.clamp(game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValue(), 10, 700)
    end)
end)()
...
function library:ShortenText(str, maxLen)
    if #str > maxLen then
...
function library:Running() return library.UI.Parent end
...
function library:BootUp()
    self:AddFolder("BlueScreen")
    self:AddFolder("BlueScreen/Caches")
    self:AddFolder("BlueScreen/Assets")
    self:AddFolder("BlueScreen/Assets/UI")
    self:AddFolder("BlueScreen/Configs")
    self:AddFolder("BlueScreen/Scripts")

    self:AddImageList("Logo", "https://i.postimg.cc/KzL4rkcd/079c51691742cbaab45c959b0b4fcba8dca6fd20-full-modified.png")
...
function library:AddImageList(Image, URL)
    if not self.ImageList[Image] then
        self.ImageList[Image] = self:AddImage("BlueScreen/Assets/UI/"..Image..".png", URL)
    end
...
-- / Config Functions
function library:LoadConfig(ConfigName,List)
    ConfigName = ConfigName or "Default"
    local Path = "BlueScreen/Configs/"..self.title.."/"..ConfigName..".json"
    if isfile(Path) then
        ...
        if success and type(result) == "table" then
            local Number = 1
            if #List == #result then
                for Tabnum, Tabs in pairs(List) do
                    for Componentnum, Components in pairs(Tabs) do
                        if Components.Type == "Toggle" then
                            Components:Set(result[Number])
                        elseif Components.Type == "Keybind" then
                            Components:SetKey(result[Number])
                        elseif Components.Type == "Slider" then
                            Components:Value(result[Number])
                        end
                        Number = Number + 1
                    end
                end
            end
            self.Notif:Notify("Loaded! " .. ConfigName .. " Config", 4, "success")
        else
            self.Notif:Notify("Failed to save! " .. ConfigName .. " Config", 4, "error")
        end
    end
end

function library:SaveConfig(ConfigName, List)
    ConfigName = ConfigName or "Default"
    local Path = "BlueScreen/Configs/"..self.title.."/"..ConfigName..".json"
    ...
    for Tabnum, Tabs in pairs(List) do
        for Componentnum, Components in pairs(Tabs) do
            if Components.Type == "Toggle" then
                configTable[Number] = Components.tof
            elseif Components.Type == "Keybind" then
                configTable[Number] = Components.Key
            elseif Components.Type == "Slider" then
                configTable[Number] = Components.Value
            end
            Number = Number + 1
        end
    end
    ...
    if success then
        self.Notif:Notify("Saved! " .. ConfigName .. " Config", 4, "success")
    else
        self.Notif:Notify("Failed to save! " .. ConfigName .. " Config", 4, "error")
    end
end
...
function library:MessageBox(innertext, caption, style)
...
function library:Watermark(text)
    for i,v in pairs(CoreGuiService:GetChildren()) do
        if v.Name:match("watermark_"..self.title .. tostring(Random(-100000, 999999))) then
            v:Destroy()
        end
    end
...
function library:InitNotifications(text, duration, callback)
    for i,v in next, CoreGuiService:GetChildren() do
        if v.name:match("Notifications_"..self.title) then
            v:Destroy()
        end
    end
    ...
    Notifications.Name = "Notifications_"..self.title .. tostring(Random(-100000, 999999))
...
function library:Introduction()
    ...
    hashLogo.Image = getcustomasset("BlueScreen/Assets/UI/Logo.png")
...
function library:Init(title, key)
    self.title = title or "bluescreen"

    for _,v in next, CoreGuiService:GetChildren() do
        if v.Name:match("BSOD_"..self.title) then
            v:Destroy()
        end
    end

    self.Notif = self:InitNotifications()
    
    local Lib = "https://raw.githubusercontent.com/Its3rr0rsWRLD/BlueScreen/refs/heads/main/Library.json"
    local success, response = pcall(function()
        return game:HttpGet(Lib)
    end)
    if success then
        local _, err =pcall(function()
            local jsonData = HttpService:JSONDecode(response)
            self.version = jsonData["V"]
            self.invite = jsonData["Invite"]
        end)
    else
        self.Notif:Notify("Something went wrong!", 4, "error")
    end
    ...
    local introduction = Instance.new("ScreenGui")
    introduction.Name = "BSOD_".. self.title
    introduction.Parent = CoreGuiService
    introduction.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    ...
    March.Visible = true
    March.Image = getcustomasset("BlueScreen/Assets/UI/March.png")
    ...
    screen.Name = "BSOD_" .. tostring(Random(-100000, 999999))
    ...
    function TabLibrary:NewTab(title)
        ...
        function Components:NewToggle(text, default, callback)
            ...
            ToggleFunctions.tof = default
            if default then
                ToggleFunctions.tof = true
            else
                ToggleFunctions.tof = false
            end
            toggleButton.MouseButton1Click:Connect(function()
                ToggleFunctions.tof = not ToggleFunctions.tof
                local SizeOn = ToggleFunctions.tof and UDim2.new(0, 12, 0, 12) or UDim2.new(0, 0, 0, 0)
                local Transparency = ToggleFunctions.tof and 0 or 1
            ...
            function ToggleFunctions:AddKeybind(default_t)
                ...
                ExtraKeybindFunctions.Key = default_t.Name
                ...
                keybind.MouseButton1Click:Connect(function()
                    ...
                    if UserInputService.WindowFocused and InputWait.KeyCode.Name ~= "Unknown" then
                        ...
                        ExtraKeybindFunctions.Key = InputWait.KeyCode.Name
                    end
                end)
                ...
                if UserInputService.WindowFocused then
                    UserInputService.InputBegan:Connect(function(c, p)
                        if not p then
                            if c.KeyCode.Name == ExtraKeybindFunctions.Key and not ChatInputBarConfiguration.IsFocused then
                                ToggleFunctions.tof = not ToggleFunctions.tof
                                local SizeOn = ToggleFunctions.tof and UDim2.new(0, 12, 0, 12) or UDim2.new(0, 0, 0, 0)
                                local Transparency = ToggleFunctions.tof and 0 or 1
                                TweenService:Create(toggleDesign, TweenTable["toggle_form"], {Size = SizeOn}):Play()
                                TweenService:Create(toggleDesign, TweenTable["toggle_form"], {BackgroundTransparency = Transparency}):Play()
                                callback_t(ToggleFunctions.tof)
                                return
                            end
                        end
                    end)
                ...
                function ExtraKeybindFunctions:SetKey(new)
                    new = new or ExtraKeybindFunctions.Key.Name
                    ExtraKeybindFunctions.Key = new.Name
                    keybindButtonLabel.Text = new.Name
                    return ExtraKeybindFunctions
                end
                --
                function ExtraKeybindFunctions:Fire()
                    callback_t(ExtraKeybindFunctions.Key)
                    return ExtraKeybindFunctions
                end
                --
                ...
                ExtraKeybindFunctions.Type = "Keybind"

                ComponentsList[ComponentsListNumber] = ExtraKeybindFunctions
                ComponentsListNumber = ComponentsListNumber + 1
            ...
            ToggleFunctions.Type = "Toggle"

            ComponentsList[ComponentsListNumber] = ToggleFunctions
            ComponentsListNumber = ComponentsListNumber + 1
        ...
        function Components:NewKeybind(text, default, callback)
            ...
            keybindButton.MouseButton1Click:Connect(function()
                ...
                if UserInputService.WindowFocused and InputWait.KeyCode.Name ~= "Unknown" then
                    local Result = Shortcuts[InputWait.KeyCode.Name] or InputWait.KeyCode.Name
                    keybindButtonLabel.Text = Result
                    KeybindFunctions.Key = InputWait.KeyCode.Name
                end
            end)
            ...
            if UserInputService.WindowFocused then
                UserInputService.InputBegan:Connect(function(c, p)
                    if not p then
                        if c.KeyCode.Name == KeybindFunctions.Key and not ChatInputBarConfiguration.IsFocused then
                            callback(KeybindFunctions.Key)
                            return
                        end
                    end
                end)
            ...
            function KeybindFunctions:SetKey(new)
                new = new or KeybindFunctions.Key.Name
                KeybindFunctions.Key = new.Name
                keybindButtonLabel.Text = new.Name
                return KeybindFunctions
            end
            --
            ...
            KeybindFunctions.Type = "Keybind"

            ComponentsList[ComponentsListNumber] = KeybindFunctions
            ComponentsListNumber = ComponentsListNumber + 1
        ...
        function Components:NewSlider(text, suffix, compare, compareSign, values, callback)
            ...
            SliderFunctions.Value = values.default
            local slideText = compare and SliderFunctions.Value .. compareSign .. tostring(values.max - 1) .. suffix or SliderFunctions.Value .. suffix
            ...
            local function UpdateSlider()
                ...
                SliderFunctions.Value = math.floor((((tonumber(values.max) - tonumber(values.min)) / sliderBackground.AbsoluteSize.X) * sliderIndicator.AbsoluteSize.X) + tonumber(values.min)) or 0.00
                ...
                pcall(function()
                    callback(SliderFunctions.Value)
                end)

                sliderValue.Text = slideText

                moveconnection = Mouse.Move:Connect(function()
                    SliderFunctions.Value = math.floor((((tonumber(values.max) - tonumber(values.min)) / sliderBackground.AbsoluteSize.X) * sliderIndicator.AbsoluteSize.X) + tonumber(values.min))
                    
                    slideText = compare and SliderFunctions.Value .. compareSign .. tostring(values.max - 1) .. suffix or SliderFunctions.Value .. suffix
                    sliderValue.Text = slideText
            ...
            function SliderFunctions:Min(new)
                ...
                values.min = new
                slideText = compare and new .. compareSign .. tostring(values.max - 1) .. suffix or SliderFunctions.Value .. suffix
                TweenService:Create(sliderIndicator, TweenTable["slider_drag"], {Size = UDim2.new(0, math.clamp(Mouse.X - sliderIndicator.AbsolutePosition.X, 0, sliderBackground.AbsoluteSize.X), 0, 12)}):Play()
                return SliderFunctions
            end
            --
            ...
            SliderFunctions.Type = "Slider"

            ComponentsList[ComponentsListNumber] = SliderFunctions
            ComponentsListNumber = ComponentsListNumber + 1
    ...
    if self.Invite ~= "" then
        task.spawn(function()
            local ReturnCode = library:MessageBox("Would you like to join our discord server?", "Discord Invite", 4)
            if ReturnCode == 6 then
                library:Invite()
                self.Notif:Notify("Success!", 4, "success")
            end
        end)
    end
    self.list = Tabs
...
function library:ConfigPanel(Tab)
    local name = "Config"
    Tab:NewSection("Config Save")
    Tab:NewTextbox("Config","","","all","medium",true,false,function(value)
        name = value
    end)
    Tab:NewButton("Save Config", function()
        self:SaveConfig(name,library.list)
    end)
    Tab:NewSection("Config Load")
    local loadi = "Config"
    Tab:NewTextbox("Config","","","all","medium",true,false,function(value)
        loadi = value
    end)
    Tab:NewButton("Load Config", function()
        self:LoadConfig(loadi,library.list)
    end)
end

function library:UIsettings(UI, Tab)
    local WM = self:Watermark(self.title .. " | v" .. self.version ..  " | " .. self:GetTime("12h") .. ":" .. self:GetTime("minute") .. " " .. self:GetTime("half") .. " | FPS: " .. self.fps)
    coroutine.wrap(function()
        while task.wait(.75) do
            WM:Text(self.title .. " | v" .. self.version ..  " | " .. self:GetTime("12h") .. ":" .. self:GetTime("minute") .. " " .. self:GetTime("half") .. " | FPS: " .. self.fps)
        end
    end)()
    Tab:NewSection("UI Settings")
    Tab:NewToggle("Watermark", true, function(value)
        if value then
            WM:Show()
        else
            WM:Hide()
    ...
    Tab:NewKeybind("UI Keybind",  UI.key, function(key)
        UI:UpdateKeybind(Enum.KeyCode[key])
    end)
end

function library:Credits(Tab)
    local Credits = Tab:NewSection("Credits")
    Tab:NewLabel("0x1000007E - Founder / Lead Developer", "left")
    Tab:NewLabel("3rr0r - Lead Developer", "left")
end
